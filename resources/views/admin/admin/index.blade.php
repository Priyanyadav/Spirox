    ...
    <div class="subheader py-2 py-lg-4  subheader-solid " id="kt_subheader">
        <div class=" container-fluid  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
            ...
            <div class="d-flex align-items-center flex-wrap mr-2">
                ...
                <div class="btn-group kt-margin-l-20">
                    {{-- @DD($data->unique('designation')) --}}
                    <select name="category_filter" id="category_filter" class="form-control">
                        <option value="">-- Select Option --</option>
                        @foreach ($data->unique('role') as $row)
                            @if ($row->designation == '0')
                                <option value="0">Admin</option>
                            @elseif($row->designation == '1')
                ...
                <div class="btn-group kt-margin-l-20">
                    {{-- @DD($data->unique('designation')) --}}
                    <select name="language_filter" id="language_filter" class="form-control">
                        <option value="">-- Select Option --</option>
                        @foreach ($data->unique('language') as $row)
                            @if ($row->language == '0')
                                <option value="0">Laravel</option>
                            @elseif($row->language == '1')
                                <option value="1">Asp.net</option>
                            @elseif ($row->language == '2')
                                <option value="2">React Js</option>
                            @elseif ($row->language == '3')
                                <option value="3">WorkPress</option>
                            @endif
                        @endforeach
                    </select>
                </div>
            ...
            <div class="d-flex align-items-center">
                <a href="{{ route('admin.admin.create') }}" data-title="Create"
                    class="btn btn-light-primary font-weight-bolder btn-sm modal-link">
                    Add
                </a>
            </div>
            <!--end::Toolbar-->
...
@push('script-push')
    <script>
        $(document).on("submit", "#adminForm", async function(e) {
            e.preventDefault();
        ...
        $(document).on("click", ".role-change", async function(e) {
            e.preventDefault();
            var data = await ajaxDynamicMethod($(this).attr('href'), "GET");
            if (data.success) {
                toastrsuccess(data.msg);
        ...
        $(document).on("click", ".delete-data", async function(e) {
            e.preventDefault();
            ...
            var dtable = $('#table').DataTable({
                ...
                ajax: {
                    ...
                    data: function(d) {
                        ...
                        console.log($('#language_filter').val());
                        d.language_filter = $('#language_filter').val();
                    }
                },
                columns: [{
                        data: 'DT_RowIndex',
